...
const MAXFPS = 7;
const minimumExecutionTime = 1000 / MAXFPS;
let lastExecutiontime = 0;

exports.grabFrames = (videoFile, delay, onFrame) => {
    const cap = new cv.VideoCapture(videoFile);
    readAndDetect(cap, onFrame);
};

function readAndDetect(cap, onFrame) {
    const timeLeft = minimumExecutionTime - lastExecutiontime;
    let detectionDelay = timeLeft > 0 ? timeLeft : 0;
    console.log('delay: ', detectionDelay);
    setTimeout(() => {
        let frame = cap.read();
